...
function App() {
  ...
  const carregarFluxos = async () => {
    try { const { data } = await fluxoService.listar(); setFluxos(data); }
  ...
  useEffect(() => { carregarFluxos(); carregarConexoes(); }, []);
  useEffect(() => { const i = setInterval(carregarConexoes, 10000); return () => clearInterval(i); }, []);

  const handleEditar = (f) => { setEditando(f); setModoEditor(true); };
  const handleNovo = () => {
    setEditando({ nome: '', gatilhos: '', conexaoId: null, mapa: { nodes: [{ id: 'start', type: 'startNode', position: { x: 300, y: 0 }, data: {} }], edges: [] } });
    setModoEditor(true);
  };
  const handleSalvar = async (dados) => {
    try {
      ...
      setModoEditor(false); setEditando(null); carregarFluxos();
    } catch (e) { console.error(e); }
  };
  const handleDeletar = async (id) => { if (!confirm('Remover?')) return; await fluxoService.deletar(id); carregarFluxos(); };
  const handleToggle = async (f) => { await fluxoService.atualizar(f.id, { ativo: !f.ativo }); carregarFluxos(); };
  const handleDuplicar = async (id) => { await fluxoService.duplicar(id); carregarFluxos(); };
  ...
  return (
    <div className="min-h-screen flex font-['Barlow']">
      {/* ─── SIDEBAR ─── */}
      <aside className="w-[220px] bg-white border-r border-[var(--border)] flex flex-col shrink-0">
        {/* Brand */}
        ...
        {pagina === 'fluxos' && (
          <div className="max-w-5xl mx-auto px-8 py-8">
            ...
            <div className="grid grid-cols-3 gap-4 mb-8 stagger">
              ...
              <div className="stat-card animate-fadeIn">
                <p className="stat-value text-[#2563eb]">{fluxos.reduce((a, f) => a + (f.mapa?.nodes?.length || 0), 0)}</p>
            ...
            {loading ? (
              ...
              <div className="text-center py-24 animate-fadeIn">
                <div className="w-16 h-16 bg-[var(--surface-sunken)] rounded-2xl flex items-center justify-center mx-auto mb-4 border border-[var(--border-light)]">
                ...
                <p className="text-[var(--text-secondary)] text-base font-semibold">Nenhum fluxo criado</p>
                <p className="text-[var(--text-muted)] text-sm mt-1">Comece criando seu primeiro fluxo</p>
              </div>
            ) : (
                  ...
                  <div key={fluxo.id} className="card card-interactive animate-fadeIn p-5">
                          ...
                          >
                            ...
                            {conexoes.map((c) => (
                              <option key={c.id} value={c.id}>
                                {c.nome}{c.apelido ? ` — ${c.apelido}` : ''}{c.status?.info?.wid?.user ? ` (${c.status.info.wid.user})` : ''}{c.status?.connected ? ' ✓' : ''}
                              </option>
                            ))}
                          </select>
                          {fluxo.horarioInicio && (
                            <span className="text-[11px] text-[var(--text-muted)] font-medium">{fluxo.horarioInicio} — {fluxo.horarioFim}</span>
                          )}
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {fluxo.gatilhos.split(',').map((g, i) => (
                        <span key={i} className="text-[11px] font-medium bg-[var(--surface-sunken)] text-[var(--text-secondary)] px-2.5 py-1 rounded-md border border-[var(--border-light)]">{g.trim()}</span>
                      ))}
                    </div>
                  </div>
                ))}
