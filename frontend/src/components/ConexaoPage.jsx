          ...
          {Object.entries(setores).map(([setor, conns]) => (
            <div key={setor} className="card overflow-hidden animate-fadeIn">
              ...
              <div className="divide-y divide-[var(--border-light)]">
                {conns.map((conexao) => (
                  <div key={conexao.id} className={`px-6 py-4 ${conexao.status?.connected ? 'border-l-[3px] border-l-emerald-500' : 'border-l-[3px] border-l-slate-300'}`}>
                        ...
                        <div>
                            ...
                            {editandoApelido === conexao.id ? (
                              ...
                              <>
                                <span className="text-[var(--text-primary)] font-medium text-sm">
                                  {conexao.apelido || (conexao.status?.connected ? conexao.status.info?.wid?.user : 'Sem nome')}
                                </span>
                          ...
                          <p className="text-xs text-[var(--text-faint)] mt-0.5">
                            {conexao.status?.connected
                              ? `${conexao.status.info?.wid?.user || ''} — ${conexao.status.info?.platform || 'WhatsApp'}`
                    ...
                    {conexao.status?.qrCode && !conexao.status?.connected && (
                      <div className="mt-4 bg-[var(--surface-sunken)] rounded-2xl p-6 text-center">
                        <p className="text-sm text-[var(--text-muted)] mb-3 font-medium">Escaneie o QR Code</p>
                        <div className="card p-4 inline-block">
                          <img src={conexao.status.qrCode} alt="QR Code" className="w-48 h-48" />
                        </div>
                        <p className="text-[var(--text-faint)] text-xs mt-3">WhatsApp → Aparelhos conectados → Conectar</p>
                      </div>
                    )}
                  </div>
                ))}
